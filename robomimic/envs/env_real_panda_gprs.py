...
from robomimic.envs.camera_redis_interface import CameraRedisSubInterface
...
class EnvRealPandaGPRS(EB.EnvBase):
    ...
    def __init__(
        ...
        for c_id, c_name in enumerate(self.camera_names_to_sizes):
            cr_interface = CameraRedisSubInterface(camera_id=0, use_depth=self.use_depth_obs)
            cr_interface.start()
